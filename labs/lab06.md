...
You have learned how to chain multiple requests to simulate multiturn conversations with your skill in one user session.
...
**6.1.1.** Open [./scripts/lab06.yml](../scripts/lab06.yml). In the configuration section update the missing parameter _skillId_ like you did in [lab05.yml](../scripts/lab05.yml).

The _Launch_ node is still the entry point for the test client but it is not the only node anymore. A second top-level node called GuessFive has been added. The Launch node is referencing to it via a YAML anchor.
...
**6.1.2.** In line 9 there's an intent name missing. Please set it correctly according to what makes sense in this case. Looking at the interaction model of the skill might help you get the correct name.
